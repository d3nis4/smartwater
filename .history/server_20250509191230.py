...
def get_weather_forecast(lat, lon):
    try:
        # Construiește URL-ul cu coordonatele pentru a obține prognoza pe ore
        params = {
            'key': WEATHER_API_KEY,
            'q': f'{lat},{lon}',  # folosim coordonatele lat, lon
            'days': 3,  # Obținem prognoza pentru 3 zile
            'hourly': 'temp,precip',  # Cerem doar temperatura și precipitațiile pe oră
        ...
        if response.status_code == 200:
            ...
            # Extrage prognoza pe ore pentru următoarele 3 zile
            forecast_data = []
