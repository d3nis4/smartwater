...
WEATHER_MAPPING = {
    ...
    "blowing snow": "Cloudy", "patchy snow possible": "Cloudy", "patchy sleet possible": "Cloudy",
    ...
    "light showers of ice pellets": "Cloudy", "moderate or heavy showers of ice pellets": "Cloudy",
...
def predict_from_firebase():
    ...
    try:
        ...
        # Ensure moisture_firebase and temperature_firebase are numeric
        try:
        ...
        # Condition 1: Unknown weather condition
        if standardized_condition == "Unknown":
            use_gradient_boosting = True
            print("[INFO] Condiție meteo necunoscută. Se va folosi modelul Gradient Boosting.")
        
        # Condition 2: Temperature from Firebase is outside normal operating range (below 10 or above 45)
        ...
        # Condition 3: Moisture from Firebase deviates significantly from the 28-45 range
        # Lower bound: 28 - 5 = 23
        # Upper bound: 45 + 5 = 50
        ...
        if moisture_firebase > 87:
            ...
            if model_name == "ANN":
                ...
                # If 'Unknown' was not mapped, it will be -1. ANN might not handle -1 well.
                # If standardized_condition is "Unknown", we should have already selected Gradient Boosting,
                # but adding a check for robustness if somehow ANN is still selected.
