...
export const calculateDuration = (startTime, endTime) => {
  ...
  if (durationMinutes <= 0) {
    // Dacă durata e 0 sau negativă, nu afișăm nimic
    return "";
  }
  ...
  // Construim un array de părți pe care apoi le unim
  const parts = [];
  ...
  if (minutes > 0) {
    // Formatare pluralizare corectă pentru minute
    if (minutes === 1) {
      parts.push("un minut");
    } else if (minutes >= 20) {
      parts.push(`${minutes} de minute`);
    } else {
      parts.push(`${minutes} minute`);
    }
  }

  // Întoarcem textul formatat, de exemplu "1 oră și 15 minute"
  return parts.join(" și ");
};

export const formatDuration = (totalMinutes) => {
  const rounded = Math.round(totalMinutes); // rotunjim la cel mai apropiat întreg
  if (rounded <= 0) return "0 minute";
