...
export const isDayTimeFromDateTime = (dateTime, forecastDays) => {
  ...
  const dateString = date.toISOString().split("T")[0]; // "2025-06-15"

  const forecastDay = forecastDays.find(day => day.date === dateString);
  if (!forecastDay || !forecastDay.astro?.sunrise || !forecastDay.astro?.sunset) {
    return "Zi"; // fallback dacă nu avem date
  }

  // Convertor AM/PM în format 24h
  ...
  // Creează date complete pentru sunrise și sunset
  const { hours: sunriseHours, minutes: sunriseMinutes } = convertTo24h(forecastDay.astro.sunrise);
  const { hours: sunsetHours, minutes: sunsetMinutes } = convertTo24h(forecastDay.astro.sunset);
  ...
  // Compară ora curentă cu intervalul [sunrise, sunset)
