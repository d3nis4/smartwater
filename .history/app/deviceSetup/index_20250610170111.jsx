...
// NEW FUNCTION: Listen for Firebase connection status
useEffect(() => {
    if (bleConfigSent && user?.email) {
        const db = getDatabase();
        const connectionStatusRef = ref(db, `users/${safeEmail}/connectionStatus`);

        // Use onValue to listen for changes
        const unsubscribeFirebase = onValue(connectionStatusRef, (snapshot) => {
            const status = snapshot.val();
            console.log("Firebase connectionStatus:", status); // Debugging line
            if (status === "connected") {
                setIsConfigured(true); // Now we can say it's truly configured
                Alert.alert("Succes", "Dispozitiv configurat și conectat la Firebase!");
                setBleConfigSent(false); // Reset this flag
                setIsConnecting(false); // Stop spinner
                // No need to unsubscribe here, as onValue will continue to listen
                // or you might want to unsubscribe if you navigate away.
                // For now, let's keep it listening as the device might disconnect later.
            } else if (status === "disconnected") {
                // Handle case where device disconnects later, if needed
                // For initial setup, we care about 'connected'
                setIsConfigured(false); // If it was connected and then disconnects, reset
                // You might want to show an alert here as well for disconnection
            }
        });
        ...
        return () => {
            ...
            // This is important to prevent memory leaks and unnecessary listeners
        };
    }
}, [bleConfigSent, user?.email, safeEmail]); // Re-run if these dependencies change

const handleConfigureDevice = async () => {
    try {
        setIsConnecting(true); // Start spinner
        setBleConfigSent(false); // Ensure this is false initially
        setIsConfigured(false); // Ensure this is false initially
        ...
        if (!hasPermissions) {
            ...
            setIsConnecting(false); // Stop spinner on permission error
            return;
        }
        ...
        // Success: BLE payload sent. Now wait for Firebase confirmation.
        setBleConfigSent(true); // Trigger the useEffect listener
        Alert.alert("Configurație trimisă", "Dispozitivul încearcă să se conecteze la Wi-Fi și Firebase..."); // Inform user
        // Keep isConnecting true until Firebase status is 'connected'
        ...
        bleManager.stopDeviceScan();
        // Do NOT set setIsConnecting(false) here. It will be set by the Firebase listener.
        // It should only be set to false if the BLE process itself fails (e.g., scan timeout).
        if (!bleConfigSent) { // If BLE failed before sending config, then stop spinner
...
const handleBothActions = async () => {
  ...
  if (!locationCoords?.lat || !locationCoords?.lon) {
    Alert.alert("Eroare", "Coordonatele locației lipsesc. Selectează locația din listă.");
    return;
  }

  await handleConfigureDevice();
  await saveDeviceLocation(); // 👉 APELĂM AICI SALVAREA LOCAȚIEI
};
  ...
  useEffect(() => {
    const backAction = () => {
      Alert.alert("Renunți la configurare?", "Vrei să revii la ecranul principal?", [
        {
          text: "Nu",
          onPress: () => null,
          style: "cancel",
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <Text style={styles.loginTitle}>Conectează dispozitivul la rețeaua ta</Text>
    </View>

    {/* Instruction Card */}
    <View style={styles.instructionCard}>
      <View style={styles.cardHeader}>
        <MaterialIcons name="info-outline" size={24} color={Colors.DARKGREEN} />
        <Text style={styles.instructionTitle}>Înainte de a continua</Text>
      </View>
      
      <View style={styles.instructionItem}>
        <MaterialIcons name="bluetooth" size={18} color={Colors.DARKGREEN} />
        <Text style={styles.instructionText}>Activează Bluetooth-ul pe telefon</Text>
      </View>
      
      <View style={styles.instructionItem}>
        <MaterialCommunityIcons name="water" size={18} color={Colors.DARKGREEN} />
        <Text style={styles.instructionText}>
          Selectează dispozitivul <Text style={styles.highlightText}>"SmartWater"</Text>
        </Text>
      </View>
    </View>
