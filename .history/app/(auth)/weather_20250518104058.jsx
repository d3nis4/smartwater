...
import WeatherIcon from '../../api/weathericon';
  ...
  const WeatherComponent = () => {
    ...
    const imageSource = weatherImages[weatherCondition];

    console.log('Imagine:', imageSource);

    console.log("🔍 Cheie weatherCondition:", weatherCondition);
console.log("✅ Există în weatherImages:", weatherCondition in weatherImages);
const WeatherIcon = weatherImages[weatherCondition];
    const getTemperatureColors = (tempC) => {
      ...
      const colorPalettes = {
        hot: [
          ...
          'hsla(12, 52.80%, 79.20%, 0.80)'     // Coral deschis
        ],
        warm: [
...
const conditionKey =
  weather?.forecast?.forecastday?.[0]?.day?.condition?.text?.trim();

console.log("🔍 Condiție meteo selectată:", conditionKey);
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', paddingHorizontal: 20 }}>
                      ...
                      <View style={{ flex: 1,marginBottom:20}}>
                        ...
                        <View style={{ alignItems: 'flex-start' }}>
                          <WeatherIcon condition={weather?.forecast?.forecastday[1]?.day?.condition?.text?.trim();} />weather?.forecast?.forecastday[1]?.day?.condition?.text?.trim();
                        </View>
                      </View>
