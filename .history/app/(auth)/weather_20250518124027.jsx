...
function renderHourlyForecast(forecastDay) {
  if (!forecastDay) return null;

  const sunrise = forecastDay.astro?.sunrise; // ex: "06:12 AM"
  const sunset = forecastDay.astro?.sunset;   // ex: "08:45 PM"
  ...
  const sunriseHour = convertTo24Hour(sunrise);
  const sunsetHour = convertTo24Hour(sunset);
  ...
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 10 }}>
      {forecastDay.hour.map((hour, index) => {
        ...
        // Creăm cheia pentru imaginea de noapte
        const nightKey = 'Luna' + hour.condition.text.charAt(0).toUpperCase() + hour.condition.text.slice(1);

        const iconImage = isDayTime
          ? (weatherImages[hour.condition.text] || { uri: `https:${hour.condition.icon}` })
          : (weatherImages[nightKey] || weatherImages[hour.condition.text] || weatherImages['LunaSenin'] || { uri: `https:${hour.condition.icon}` });

        return (
          <View key={index} style={styles.hourlyItem}>
            <Text style={[styles.hourlyTime, dynamicStyles.text]}>{hourOfDay}:00</Text>
            <Image source={iconImage} style={styles.hourlyIcon} resizeMode="contain" />
