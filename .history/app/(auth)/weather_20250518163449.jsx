...
function isDayTimeFromDateTime(dateTimeStr, forecastDays) {
  ...
  const inputDate = new Date(parts[0], parts[1] - 1, parts[2], parts[3], parts[4]);
  ...
  // Convert sunrise/sunset to 24-hour format and get hours/minutes
  ...
  // Create Date objects for sunrise/sunset on the input date
  const sunriseDate = new Date(inputDate);
  sunriseDate.setHours(sunriseHour, sunriseMinute, 0, 0);
  
  const sunsetDate = new Date(inputDate);
  sunsetDate.setHours(sunsetHour, sunsetMinute, 0, 0);
console.log(
  `🌅 Sunrise: ${sunriseDate.toLocaleString()} | 🌇 Sunset: ${sunsetDate.toLocaleString()} | 🕒 Current: ${inputDate.toLocaleString()}`
);
  // Compare the times
