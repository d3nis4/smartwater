...
export const LogoutButton = () => {
  ...
  const { userLoggedIn, espConnected } = useAuth(); 
  const doLogout = () => {
    signOut();
  };
...
const TabsPage = () => {
 ...
 return (
    ...
    >
      {/* Dacă nu ești conectat la ESP32, afișează config în loc de home */}
      {!espConnected && userLoggedIn && (
        <Tabs.Screen
          name="config"
          options={{
            headerTitle: 'Configurare ESP32',
      ...
      {/* Dacă ești conectat, afișează home */}
      {espConnected && userLoggedIn && (
        <Tabs.Screen
          name="home"
          options={{
