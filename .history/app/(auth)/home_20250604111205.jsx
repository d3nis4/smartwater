  ...
  useEffect(() => {
    const checkDeviceConfigured = async () => {
      try {
        ...
        if (snapshot.exists()) {
          setIsConfigured(snapshot.val()); // true sau false
        } else {
          setIsConfigured(false); // implicit
        }
      } catch (err) {
        console.error("Eroare la verificare configurare:", err);
        setIsConfigured(false); // fallback
      } finally {
        setLoading(false);
      }
    };
