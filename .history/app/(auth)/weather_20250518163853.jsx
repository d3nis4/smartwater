...
function isDayTimeFromDateTime(dateTimeStr, forecastDays) {
  // Parsează data și ora din string
  ...
  // Convertirea orei de răsărit/apus 12h AM/PM la 24h
  function convertTo24Hour(time12h) {
    const [time, modifier] = time12h.split(' ');
    let [hours, minutes] = time.split(':').map(Number);
  ...
  // Construim datele de răsărit și apus în UTC
  const sunriseDate = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2], sunriseTime.hours, sunriseTime.minutes));
  const sunsetDate = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2], sunsetTime.hours, sunsetTime.minutes));

  console.log(`Input date:   ${inputDate.toISOString()}`);
  console.log(`Sunrise UTC:  ${sunriseDate.toISOString()}`);
  console.log(`Sunset UTC:   ${sunsetDate.toISOString()}`);

  // Verificăm dacă inputul e între răsărit și apus
