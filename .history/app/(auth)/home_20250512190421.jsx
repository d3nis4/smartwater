 ...
 const fetchSmartPrediction = async (safeEmail) => {
  ...
  try {
    const response = await fetch(
      ...
      { timeout: 10000 } // opțional: setează timeout dacă folosești o bibliotecă ca axios
    );
    ...
    if (data?.prediction !== undefined) {
      ...
      throw new Error("Răspuns invalid de la serverul Flask");
    }
  } catch (error) {
    console.error("Smart prediction error:", error);

    // Fallback simplu: dacă umiditatea < prag, pornește pompa
    try {
      const moistureSnapshot = await get(ref(db, `users/${safeEmail}/soilHumidity`));
      const moistureValue = moistureSnapshot.exists() ? moistureSnapshot.val() : null;

      if (moistureValue !== null) {
        const shouldTurnOn = moistureValue < autoThreshold;
        console.warn("Folosește fallback: prag local pentru umiditate");
      ...
      console.error("Eroare fallback:", fallbackError);
    }
  }
};
  ...
  const moistureUnsub = onValue(moistureRef, () => fetchSmartPrediction(safeEmail));
      ...
      <View style={styles.pumpContainer}>
        ...
        <View style={styles.modeSelector}>
          ...
          <TouchableOpacity
            ...
            disabled={!savedLocation} // opțional: dezactivează complet butonul
          >
            <Ionicons
              name="settings"
              size={20}
