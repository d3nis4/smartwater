...
const Home = () => {
 ...
 useEffect(() => {
    ...
    // Setează intervalul pentru a verifica la fiecare secundă
    const interval = setInterval(() => {
      // Obține statusul actualizat al pompei
      get(statusRef)
        .then((snapshot) => {
          const status = snapshot.val();
          if (status) {
            setPumpStatus(status);  // Actualizează statusul în UI
          }
        })
        .catch((error) => {
          console.error("Eroare la obținerea statusului pompei:", error);
        });
    }, 1000);  // Verifică fiecare secundă
        ...
        <View style={styles.sectionHeader}>
          ...
          <View
            style={[
              ...
              pumpStatus?.pumpStatus === "on" ? styles.pumpOn : styles.pumpOff,
            ]}
          >
            <Text style={styles.pumpStatusText}>
              {pumpStatus?.pumpStatus === "on" ? "ACTIVĂ" : "INACTIVĂ"}
            </Text>
          </View>
        </View>
