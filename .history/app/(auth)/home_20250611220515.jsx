...
export default function Home() {
  ...
  const [autoThreshold, setAutoThreshold] = useState("N/A");
...
useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
        ...
        if (!user?.email) {
            // Clear states if user logs out or email is unavailable
            setMoisture(null);
            setTemperature(null);
            setRain(null);
            setPumpStatus("off");
            ...
            setAutoThreshold(30); // Consistent default
            setOverrideActive(false);
            setScheduledDays([]); // Clear scheduled days
            setPumpData(null);
            setSavedLocation(null);
            setIsConfigured(false); // Assume not configured if no user
            setSchedule(Array(7).fill().map(() => ({ timeSlots: [{ startTime: "", endTime: "" }] })));
            setSavedSchedule(Array(7).fill().map(() => ({ timeSlots: [{ startTime: "", endTime: "" }] })));
            return;
        }
        ...
        const unsubscribeRealtime = onValue(userRef, (snapshot) => {
            const data = snapshot.val() || {}; // Ensure data is an object even if null

            // --- Update Sensor Data ---
            setMoisture(data?.soilHumidity ?? null); // Use optional chaining and nullish coalescing
            setTemperature(data?.temperature ?? null);
            setRain(data?.rain ?? null);
            // Note: You have 'soilHumidity' and 'temperature' directly under userRef,
            // but then in the last useEffect you have `users/${safeEmail}/sensors/moisture`.
            // **Crucial: Ensure your Firebase path is consistent!**
            // If sensors are under a 'sensors' node, adjust the main listener to `data?.sensors?.moisture`.
            // Assuming `soilHumidity` and `temperature` are at the top level for now.

            // --- Update Control Settings ---
            if (data.controls) {
                setPumpMode(data.controls.pumpMode || "manual");
                setPumpStatus(data.controls.pumpStatus || "off");
                setAutoThreshold(data.controls.pragUmiditate || 30); // Consistent default
                setOverrideActive(data.controls.override || false);
                setSavedPumpMode(data.controls.pumpMode || "manual");
            } else {
                // Reset if controls node is missing
                setPumpMode("manual");
                setPumpStatus("off");
                setAutoThreshold(30);
                setOverrideActive(false);
            ...
            if (data.program) {
                ...
                // If you have a 'savedSchedule' in Firebase, update that too.
                // Assuming savedSchedule is based on local schedule, set it after schedule is updated.
                setSavedSchedule(newSchedule);
            } else {
                setSchedule(Array(7).fill().map(() => ({ startTime: "", endTime: "" })));
                setSavedSchedule(Array(7).fill().map(() => ({ startTime: "", endTime: "" })));
            }

            // --- Update Location ---
            setSavedLocation(data?.location ?? null); // If location is directly under userRef

            // --- Update Device Configuration ---
            setIsConfigured(data?.deviceConfigured ?? false); // Default to false if not present
        });

        // Cleanup for the realtime listener
        return unsubscribeRealtime;
    });

    // Cleanup for the auth listener
  ...
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      ...
      const unsubscribeRealtime = onValue(userRef, (snapshot) => {
        ...
        setTemperature(data?.temperature ?? null);
        console.log("Rain from db:", data?.rain, typeof data?.rain);
        setRain(data?.rain ?? null);
