...
import { getSafeEmail,fetchSavedLocation } from "../../constants/functions";
...
export default function Home() {
  ...
  useEffect(() => {
    if (!user?.email) return;
    fetchSavedLocation(safeEmail, db);
    const db = getDatabase();
    const safeEmail = getSafeEmail(user.email);
    const statusRef = ref(db, `users/${safeEmail}/controls/pumpStatus`);
  ...
  const handleRefresh = async () => {
    ...
    await fetchSavedLocation(safeEmail, db);
  ...
  useEffect(() => {
    const unsubscribe = navigation.addListener("focus", () => {
      fetchSavedLocation(safeEmail, db); // încarcă din Firebase când Home revine activ
    });
