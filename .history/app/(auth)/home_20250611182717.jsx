...
import { ref, get, set, getDatabase, onValue, update,useRef } from "firebase/database";
...
export default function Home() {
  ...
  // --- Efect pentru permisiuni de notificări push ---
  useEffect(() => {
    async function registerForPushNotificationsAsync() {
      // Verifică dacă rulează pe un dispozitiv fizic (nu pe simulator/emulator)
      if (Constants.isDevice) {
        ...
        if (finalStatus !== 'granted') {
          Alert.alert('Eroare Notificări', 'Nu s-au acordat permisiuni pentru notificări!');
          return;
        }
      } else {
        // Pentru simulator/emulator, notificările ar putea să nu funcționeze corect fără configurare specială.
        console.log('Notificările push trebuie rulate pe un dispozitiv fizic pentru a funcționa corect.');
      }
    }
    registerForPushNotificationsAsync();
  }, []); // Rulează o singură dată la montarea componentei

  // ... restul useEffect-urilor și a handler-ilor

  // Funcția pentru trimiterea notificărilor imediate
  async function sendNotification(message) {
    await Notifications.scheduleNotificationAsync({
      content: {
        title: "SmartWater Alert",
        body: message,
      },
      trigger: null, // Trimite notificarea imediat
    });
  }
  useEffect(() => {
  async function registerForPushNotificationsAsync() {
    try {
      if (!Constants.isDevice) {
        console.log("Notificările funcționează doar pe un dispozitiv fizic.");
        return;
      }
      ...
      if (finalStatus !== 'granted') {
        Alert.alert('Eroare Notificări', 'Permisiunea pentru notificări nu a fost acordată.');
        return;
      }
      ...
      // TODO: poți salva tokenul în Firebase aici dacă vrei
    } catch (error) {
      console.error("Eroare la înregistrarea notificărilor:", error);
    }
  }
