...
filterDataByRange(filteredLogs, selectedRange);
useEffect(() => {
...
{stats && (
  <View style={styles.statusCard}>
    ...
    <Text style={styles.bullet}>• Număr total de irigări: {stats.totalIrigari}</Text>
    <Text style={styles.bullet}>• Durată totală: {Math.floor(stats.durataTotala / 60)} ore și {stats.durataTotala % 60} minute</Text>
    <Text style={styles.bullet}>• Durată medie per irigare: {stats.durataMedie} minute</Text>
    {stats.ziMaxIrigari && (
      <Text style={styles.bullet}>• Ziua cu cele mai multe irigări: {formatDateReadable(stats.ziMaxIrigari)}</Text>
    )}
  </View>
)}
...
<View style={{ flexDirection: 'row', justifyContent: 'center', marginBottom: 10 }}>
  {[7, 30].map((days) => (
    ...
    >
      <Text style={{ color: selectedRange === days ? 'white' : '#34495e', fontWeight: 'bold' }}>
        Ultimele {days} zile
      </Text>
    </TouchableOpacity>
  ))}
