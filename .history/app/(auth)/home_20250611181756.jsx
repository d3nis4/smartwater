...
export default function Home() {
  ...
  useEffect(() => {
    // Cere permisiune pentru notificari
  ...
  useEffect(() => {
    ...
    async function checkPumpStatus() {
      try {
        const response = await fetch(`${FIREBASE_URL}/users/${userId}/controls/pumpStatus.json`);
        const data = await response.json();
        if (data !== previousPumpStatus.current) {
          // Daca s-a schimbat statusul pompei, trimite notificare
          if (previousPumpStatus.current !== null) { // nu la prima incarcare
