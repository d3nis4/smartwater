...
useEffect(() => {
  registerForPushNotificationsAsync().then(token => {
    if (token) {
      // Salvează tokenul într-un backend sau Firebase
      console.log('Tokenul dispozitivului:', token);
    }
  });
}, []);
...
setRain(data?.rain ?? null);
 ...
 useEffect(() => {
  ...
  const pumpRef = ref(db, `users/${safeEmail}/controls/pumpStatus`);
  const unsubscribe = onValue(pumpRef, async (snapshot) => {
    const status = snapshot.val();

    // Citește tokenul salvat
    const tokenRef = ref(db, `users/${safeEmail}/pushToken`);
    const tokenSnap = await get(tokenRef);
    const token = tokenSnap.val();

    if (token) {
      await sendPushNotification(token, status === "on" ? "Pompa a pornit" : "Pompa a fost oprită");
    }
  });
