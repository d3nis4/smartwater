...
const Home = () => {
  ...
  const handleTimeChange = (dayIndex, slotIndex, field, value) => {
    ...
    // Dacă există două numere și două cifre, să punem automat ":"
    let newValue = formattedValue;
    if (formattedValue.length === 2 && !formattedValue.includes(":")) {
      newValue = formattedValue + ":";
    }
  
    if (newValue.length > 5) return; // Maxim 5 caractere (HH:MM)
  
    // Verificare format corect
    const timeRegex = /^([01]\d|2[0-3]):([0-5]\d)$/;
    if (newValue.length === 5 && !timeRegex.test(newValue)) {
      // Dacă nu e valid, nu facem nimic sau putem alerta
      return;
    }
  
    const updatedSchedule = [...schedule];
    updatedSchedule[dayIndex].timeSlots[slotIndex][field] = newValue;
    setSchedule(updatedSchedule);
  
    // Verificare început < sfârșit
    ...
    if (startTime.length === 5 && endTime.length === 5) {
      ...
      if (startTotal >= endTotal) {
        ...
        // Poți să resetezi ora de sfârșit sau să lași utilizatorul să corecteze
      }
    }
  };
