...
const sunrise = new Date(weather.forecast.forecastday[0].astro.sunrise); // ex: "06:15 AM"
const sunset = new Date(weather.forecast.forecastday[0].astro.sunset);   // ex: "08:30 PM"
function isDayTime(hour) {
  // Extragem doar ora în format 24h pentru comparație
  const [sunriseHour, sunriseMin, sunriseAMPM] = weather.forecast.forecastday[0].astro.sunrise.split(/[: ]/);
  const [sunsetHour, sunsetMin, sunsetAMPM] = weather.forecast.forecastday[0].astro.sunset.split(/[: ]/);

  // Convertim răsăritul la format 24h
  let sunrise24 = parseInt(sunriseHour);
  if (sunriseAMPM === "PM" && sunrise24 !== 12) sunrise24 += 12;
  if (sunriseAMPM === "AM" && sunrise24 === 12) sunrise24 = 0;

  // Convertim apusul la format 24h
  let sunset24 = parseInt(sunsetHour);
  if (sunsetAMPM === "PM" && sunset24 !== 12) sunset24 += 12;
  if (sunsetAMPM === "AM" && sunset24 === 12) sunset24 = 0;

  // Comparăm ora dată cu răsăritul și apusul
  return hour >= sunrise24 && hour < sunset24;
}
...
{weather?.forecast?.forecastday[0]?.hour.map((hour, index) => {
  ...
  // Alegem imaginea în funcție de zi sau noapte
  const conditionText = hour.condition.text;
  let iconSource;

  if (isDay) {
    // ziua: dacă există imagine în weatherImages, o folosim
    iconSource = weatherImages[conditionText] ? weatherImages[conditionText] : { uri: `https:${hour.condition.icon}` };
  } else {
    // noaptea: modificăm textul condiției pentru a lua varianta cu luna dacă există
    // Exemplu: "Soare" devine "LunaSenin", "Noros" devine "LunaNor", etc.

    // Definim un map cu condiții pentru noapte:
    const nightConditionMap = {
      "Soare": LunaSenin,
      "Senin": LunaSenin,
      "Noros": NorLuna,
      ...
      // adaugă alte condiții după caz
    };

    iconSource = nightConditionMap[conditionText]
      ? nightConditionMap[conditionText]
...
{weather?.forecast?.forecastday[0]?.hour.map((hour, index) => {
  ...
  return (
    <View key={index} style={styles.hourlyItem}>
      <Text style={[styles.hourlyTime, dynamicStyles.text]}>{currentHour}:00</Text>
      <Image source={iconSource} style={styles.hourlyIcon} resizeMode="contain" />
      ...
      <View style={styles.precipitationContainer}>
        ...
        <Text style={[styles.precipitationLabel, dynamicStyles.text]}>{hour.condition.text}</Text>
      </View>
    </View>
  );
})}
                      ...
                      {weather?.forecast?.forecastday[0]?.hour.map(
                        (hour, index) => (
                          <View key={index} style={styles.hourlyItem}>
                            ...
                            <Image
                              source={weatherImages[hour.condition.text] 
