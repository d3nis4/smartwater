...
const WeatherComponent = () => {
  ...
  function isDayTime(hour,weather) {
    // Extragem doar ora în format 24h pentru comparație
    const [sunriseHour, sunriseMin, sunriseAMPM] =
      weather.forecast.forecastday[0].astro.sunrise.split(/[: ]/);
    const [sunsetHour, sunsetMin, sunsetAMPM] =
      weather.forecast.forecastday[0].astro.sunset.split(/[: ]/);
...
// Filtrează doar orele între acum și +24h
...
>
  {next24Hours.map((hour, index) => {
    ...
    const isDay = isDayTime(currentHour, weather);
    const conditionText = hour.condition.text;

    // Imaginea în funcție de zi/noapte
    ...
    const iconSource = isDay
      ? weatherImages[conditionText] || { uri: `https:${hour.condition.icon}` }
                      ...
                      {weather?.forecast?.forecastday[0]?.hour.map(
                        (hour, index) => {
                          ...
                          if (isDay) {
                            ...
                            iconSource = weatherImages[conditionText]
                              ? weatherImages[conditionText]
                              : { uri: `https:${hour.condition.icon}` };
                          } else {
