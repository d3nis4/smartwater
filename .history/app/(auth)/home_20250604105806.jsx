...
export default function Home() {
  ...
  const [rain, setRain] = useState(null);a  const [temperature, setTemperature] = useState(null);
...
useEffect(() => {
  ...
  console.log("Se verifică Firebase pentru:", safeEmail); // 🛠 DEBUG
  
  const unsubscribe = onValue(configRef, (snapshot) => {
    console.log("Snapshot există?", snapshot.exists()); // 🛠 DEBUG
    setIsConfigured(snapshot.exists() ? snapshot.val() : false);
    setLoading(false);
  }, (error) => {
    console.error("Eroare Firebase:", error); // 🛠 Captează erori
    setLoading(false); // Oprit loading chiar și la eroare
  });
