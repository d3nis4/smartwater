...
const [currentPicker, setCurrentPicker] = useState({ dayIndex: null, slotIndex: null, field: null });
...
const onTimeSelected = (event, selectedDate) => {
  ...
  if (selectedDate) {
    ...
    updatedSchedule[currentPicker.dayIndex].timeSlots[currentPicker.slotIndex][currentPicker.field] = timeString;
  ...
  const handleTimeChange = (dayIndex, slotIndex, field, value) => {
    ...
    // Adaugă automat ":" după două cifre
    if (formattedValue.length === 2 && !formattedValue.includes(":")) {
      formattedValue += ":";
    }
  
    if (formattedValue.length > 5) return; // Maxim 5 caractere
  
    const timeRegex = /^([01]\d|2[0-3]):([0-5]\d)$/; // Format valid
  
    // Dacă am 5 caractere, verificăm dacă este valid HH:MM
    if (formattedValue.length === 5 && !timeRegex.test(formattedValue)) {
      return; // Dacă nu este corect (ex. 45:99), ignorăm schimbarea
    }
  
    // Dacă avem 1 sau 2 caractere fără ":" verificăm că sunt între 0 și 23
    if (formattedValue.length <= 2 && !formattedValue.includes(":")) {
      const hour = parseInt(formattedValue, 10);
      if (isNaN(hour) || hour > 23) {
        return; // Dacă ora este >23, ignorăm
      }
    }
  
    const updatedSchedule = [...schedule];
    updatedSchedule[dayIndex].timeSlots[slotIndex][field] = formattedValue;
    setSchedule(updatedSchedule);
  
    // Dacă avem start și end completate, validăm ordinea
...
<TouchableOpacity
  ...
  onPress={() => showTimePicker(dayIndex, slotIndex, "startTime")}
>
...
<TouchableOpacity
  style={styles.timeInput}
  onPress={() => showTimePicker(dayIndex, slotIndex, "endTime")}
>
