...
function isDayTimeFromDateTime(dateTimeStr, forecastDays) {
  const inputDateUTC = new Date(dateTimeStr);

  // Extragem componentele din inputDateUTC și le convertim în ora locală manual
  const localYear = inputDateUTC.getUTCFullYear();
  const localMonth = inputDateUTC.getUTCMonth();
  const localDay = inputDateUTC.getUTCDate();
  const localHour = inputDateUTC.getUTCHours();
  const localMinute = inputDateUTC.getUTCMinutes();

  // Cream data locală exactă (fără fus)
  const inputDate = new Date(localYear, localMonth, localDay, localHour, localMinute);
  ...
  console.log("forecastDays dates =", forecastDays.map(d => d.date));

  const matchingDay = forecastDays.find(day => day.date === inputDateStr);

  if (!matchingDay) {
    console.log("Ziua nu a fost găsită în forecast, presupunem zi.");
    return "Zi"; // fallback
  }
  ...
  const sunriseDate = new Date(inputDate.getFullYear(), inputDate.getMonth(), inputDate.getDate(), sunriseTime.hours, sunriseTime.minutes);
  const sunsetDate = new Date(inputDate.getFullYear(), inputDate.getMonth(), inputDate.getDate(), sunsetTime.hours, sunsetTime.minutes);

  console.log("Input date local:  ", inputDate);
  console.log("Sunrise local:     ", sunriseDate);
  console.log("Sunset local:      ", sunsetDate);
...
const conditionText = weather.current.condition.text;
  ...
  const timeToCheck = "2025-05-18T07:30:00"; // exemplu, poate să fie dintr-un timestamp din forecast hourly
  const dayOrNight = isDayTimeFromDateTime(timeToCheck, forecastDays);

  console.log("Este zi sau noapte?", dayOrNight);;
  const date = new Date("2025-05-18T19:00:00.000Z");
console.log(date.toString()); // Arată ora locală
console.log(date.toISOString()); // Arată ora în UTC
