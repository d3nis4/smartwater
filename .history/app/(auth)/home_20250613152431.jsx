...
useEffect(() => {
  const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
    ...
    const unsubscribeRealtime = onValue(userRef, (snapshot) => {
      ...
      // PumpData update separat dacă folosești alt state pentru afișare
      setPumpData((prev) => ({
        ...prev,
...
useEffect(() => {
  ...
  const unsubMoisture = onValue(moistureRef, () => fetchSmartPrediction(safeEmail));
  const unsubTemp = onValue(tempRef, () => fetchSmartPrediction(safeEmail));

  fetchSmartPrediction(safeEmail); // la start
