...
function isDayTimeFromDateTime(dateTimeStr, forecastDays) {
  ...
  // Folosește `UTC` pentru a evita problemele de fus orar
  const inputDate = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2], parts[3], parts[4]));
  
  const inputDateStr = `${parts[0]}-${String(parts[1]).padStart(2, "0")}-${String(parts[2]).padStart(2, "0")}`;
  const matchingDay = forecastDays.find((day) => day.date === inputDateStr);
  if (!matchingDay) return true;

  // Convertește sunrise/sunset la ore UTC
  const sunrise24 = convertTo24Hour(matchingDay.astro.sunrise);
  const sunset24 = convertTo24Hour(matchingDay.astro.sunset);
  
  const [sunriseHour, sunriseMinute] = sunrise24.split(":").map(Number);
  const [sunsetHour, sunsetMinute] = sunset24.split(":").map(Number);

  // Creează `Date` obiecte în UTC
  const sunriseDate = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2], sunriseHour, sunriseMinute));
  const sunsetDate = new Date(Date.UTC(parts[0], parts[1] - 1, parts[2], sunsetHour, sunsetMinute));

  console.log(
    `🌅 Sunrise (UTC): ${sunriseDate.toISOString()} | ` +
    `🌇 Sunset (UTC): ${sunsetDate.toISOString()} | ` +
    `🕒 Current (UTC): ${inputDate.toISOString()}`
  );

  return inputDate >= sunriseDate && inputDate < sunsetDate;
}
