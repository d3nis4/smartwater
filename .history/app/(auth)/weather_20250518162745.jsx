...
const inputDateParts = dateTimeStr.split(/[- :]/).map(Number);
const inputDate = new Date(inputDateParts[0], inputDateParts[1] - 1, inputDateParts[2], inputDateParts[3], inputDateParts[4]);


  // Găsește ziua potrivită din forecast
  const matchingDay = forecastDays.find((day) => day.date === inputDateStr);
  if (!matchingDay) return true; // fallback: considerăm zi

  // Creează date complete pt sunrise și sunset
  const sunriseStr = `${matchingDay.date} ${convertTo24Hour(matchingDay.astro.sunrise)}`; // "2025-05-19 06:00"
  const sunsetStr = `${matchingDay.date} ${convertTo24Hour(matchingDay.astro.sunset)}`;   // "2025-05-19 20:30"

const [sunriseHour, sunriseMinute] = convertTo24Hour(matchingDay.astro.sunrise).split(":").map(Number);
const [sunsetHour, sunsetMinute] = convertTo24Hour(matchingDay.astro.sunset).split(":").map(Number);

const sunrise = new Date(matchingDay.date + "T00:00:00");
sunrise.setHours(sunriseHour, sunriseMinute);

const sunset = new Date(matchingDay.date + "T00:00:00");
sunset.setHours(sunsetHour, sunsetMinute);

  return inputDate >= sunrise && inputDate < sunset;
}
