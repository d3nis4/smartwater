...
const FIREBASE_URL = Constants.exportco
...
export default function Home() {
  ...
  const previousPumpStatus = useRef(null);
  useEffect(() => {
    async function registerForPushNotificationsAsync() {
      const { status } = await Notifications.requestPermissionsAsync();
      if (status !== "granted") {
        alert("Nu s-au acordat permisiuni pentru notificări!");
      }
    }
  ...
  useEffect(() => {
    // Functie care verifica periodic statusul pompei din Firebase
    const userId = "exempluUserId"; // TODO: pune userId-ul real

    async function checkPumpStatus() {
      try {
        const response = await fetch(
          `${FIREBASE_URL}/users/${userId}/controls/pumpStatus.json`
        );
        const data = await response.json();
        if (data !== previousPumpStatus.current) {
          // Daca s-a schimbat statusul pompei, trimite notificare
          if (previousPumpStatus.current !== null) {
            // nu la prima incarcare
            sendNotification(`Status pompa schimbat: ${data}`);
          }
          previousPumpStatus.current = data;
          setPumpStatus(data);
        }
      } catch (error) {
        console.log("Eroare la preluarea pumpStatus:", error);
      }
    }

    // Verifica la fiecare 30 de secunde
    const intervalId = setInterval(checkPumpStatus, 30000);

    // Verifica imediat si la prima rulare
    checkPumpStatus();

    return () => clearInterval(intervalId);
  }, []);

  // Functia care trimite notificarea
  async function sendNotification(message) {
    await Notifications.scheduleNotificationAsync({
      ...
      trigger: null, // notificare imediata
    });
  }
