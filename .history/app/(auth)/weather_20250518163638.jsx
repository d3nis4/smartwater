...
function isDayTimeFromDateTime(dateTimeStr, forecastDays) {
  ...
  const matchingDay = forecastDays.find(day => day.date === dateStr);
  if (!matchingDay) return true; // fallback
  ...
  // Obține ziua anterioară
  const prevDay = new Date(Date.UTC(parts[0], parts[1] -1, parts[2] - 1));
  const prevDayStr = prevDay.toISOString().slice(0, 10);
  const prevMatchingDay = forecastDays.find(day => day.date === prevDayStr);
  
  let prevSunsetDateUTC;
  if (prevMatchingDay) {
    const prevSunset24 = convertTo24Hour(prevMatchingDay.astro.sunset);
    const [prevSunsetH, prevSunsetM] = prevSunset24.split(":").map(Number);
    prevSunsetDateUTC = new Date(Date.UTC(prevDay.getUTCFullYear(), prevDay.getUTCMonth(), prevDay.getUTCDate(), prevSunsetH, prevSunsetM));
  }

  // Dacă ora este după apusul zilei precedente și înainte de răsăritul zilei curente => noapte
  if (prevSunsetDateUTC && inputDateUTC >= prevSunsetDateUTC && inputDateUTC < sunriseDateUTC) {
    return false; // noapte
  }

  // Dacă ora este între răsărit și apus => zi
