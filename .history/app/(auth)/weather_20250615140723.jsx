...
export const open_weather_key = process.env.EXPO_PUBLIC_OPEN_WEATHER_FORECAST_API_KEY;
export const open_weather_url = process.env.EXPO_PUBLIC_OPEN_WEATHER_open_weather_url;

/**
 * Componente principală pentru afișarea informațiilor meteo pentru ziua de azi, mâine și prognoză.
 * Se ocupă cu încărcarea datelor meteo și actualizarea locației.
 */
...
export default function WeatherComponent() {
  ...
  /**
   * Încarcă datele meteo extinse (prognoză pe mai multe zile) folosind coordonate.
   * @param {number} lat - Latitudinea locației.
   * @param {number} lon - Longitudinea locației.
   */
  ...
  /**
   * Încarcă datele meteo pentru un anumit oraș.
   * @param {string} cityName - Numele orașului.
   */
  ...
  /**
   * Cere permisiune și obține locația curentă a dispozitivului,
   * apoi încarcă datele meteo pentru această locație.
   */
  ...
  /**
   * Actualizează lista de locații sugerate pe baza textului introdus.
   * @param {string} text - Textul introdus în căutare.
   */
  ...
  /**
   * Setează orașul selectat din lista cu locații sugerate și încarcă datele meteo pentru acesta.
   * @param {object} city - Obiectul oraș selectat.
   */
  ...
  /**
   * Încarcă vremea inițială la pornirea aplicației,
   * fie din locația salvată în baza de date, fie dintr-un oraș implicit.
   */
  const loadInitialWeather = async () => {
    ...
    try {
      ...
      console.error("Eroare la încărcarea locației:", err);
    } finally {
      setLoading(false);
    }
  };
                ...
                >
                  ...
                  <Card style={styles.mapCard}>
                    ...
                    >
                      ...
                      {/* Precipitații */}
                      <UrlTile
                        urlTemplate={`${open_weather_url}/precipitation_new/{z}/{x}/{y}.png?appid=${open_weather_key}&lat=${
                      ...
                      {/* Temperatură */}
                      <UrlTile
                        urlTemplate={`${open_weather_url}/temp_new/{z}/{x}/{y}.png?appid=${open_weather_key}&lat=${
                          weather?.location?.lat || 44.4268
                        }&lon=${weather?.location?.lon || 26.1025}`}
                        maximumZ={19}
                        flipY={false}
...
/**
 * Stiluri pentru componentele din aplicație.
 */
