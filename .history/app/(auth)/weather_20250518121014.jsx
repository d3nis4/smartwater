...
const conditionText = weather?.forecast?.forecastday[0]?.condition?.text?.trim();
console.log('conditionText:', conditionText);

const imageFromMap = conditionText ? weatherImages[conditionText] : undefined;
console.log('imageFromMap:', imageFromMap);

const fallbackIcon = current?.condition?.icon ? `https:${current.condition.icon}` : null;
console.log('fallbackIcon:', fallbackIcon);

const sursaimagine = imageFromMap
  ? imageFromMap
  : fallbackIcon
  ? { uri: fallbackIcon }
  : null;

console.log('sursaimagine:', sursaimagine);
...
<View style={{ alignItems: 'flex-start' }}>
    {sursaimagine ? (
      <Image style={styles.weatherImage} source={sursaimagine} />
    ) : (
      <Text>Imagine indisponibilă</Text>
    )}
  </View>
                      </View>
