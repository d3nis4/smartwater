...
function isDayTimeFromDateTime(dateTimeStr, forecastDays) {
  // Creează obiectul Date direct din stringul complet, tratând fusul local implicit
  const inputDate = new Date(dateTimeStr);

  const inputDateStr = inputDate.toISOString().split('T')[0]; // format YYYY-MM-DD în UTC
  ...
  // Construim datele pentru răsărit și apus folosind ora locală (același fus orar ca inputDate)
  ...
  console.log(`Input date local:   ${inputDate.toString()}`);
  console.log(`Sunrise local:      ${sunriseDate.toString()}`);
  console.log(`Sunset local:       ${sunsetDate.toString()}`);
console.log("Zile disponibile în forecast:", forecastDays.map(d => d.date));

  return inputDate >= sunriseDate && inputDate < sunsetDate ? "Zi" : "Noapte";
}
