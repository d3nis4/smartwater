...
export default function Profile() {
  ...
  try {
    ...
    if (status !== 'granted') {
      Alert.alert("Permisiune respinsă", "Permisiunea pentru locație este necesară.");
      return;
    }
    ...
    if (reverseGeocode.length > 0) {
      ...
      const cityName = place.city || place.region || "Locație necunoscută";
      const fullCity = `${cityName}, ${place.country}`;
      
      setDeviceCity(fullCity);
      setLocationCoords({ lat: latitude, lon: longitude });

      Alert.alert("Succes", `Locație detectată: ${fullCity}`);
    }
  } catch (error) {
...
>
  ...
  <Text style={styles.buttonText}>Folosește locația mea actuală</Text>
</TouchableOpacity>
