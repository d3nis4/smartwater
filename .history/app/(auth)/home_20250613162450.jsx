...
export default function Home() {
 ...
 const [currentPickingInfo, setCurrentPickingInfo] = useState({
  dayIndex: null,
  slotIndex: null,
  fieldName: null, // 'startTime' sau 'endTime'
  initialPickerDate: new Date(), // Adăugăm o proprietate pentru data inițială a picker-ului
});
  ...
  // Preia valoarea existentă a orei din slot (ex: "09:30")
  const existingTimeString = schedule[dayIndex].timeSlots[slotIndex][fieldName];

  let initialDateForPicker;
  if (existingTimeString && existingTimeString !== "HH:MM") { // Verificăm dacă există o oră validă
    // Dacă există o oră salvată, o parsez într-un obiect Date
    const [hours, minutes] = existingTimeString.split(':').map(Number);
    initialDateForPicker = new Date(); // Pornim cu data curentă (important pentru consistență)
    initialDateForPicker.setHours(hours, minutes, 0, 0); // Setăm orele și minutele existente
  } else {
    // Dacă slotul este gol, poți alege o valoare implicită:
    initialDateForPicker = new Date(); // Deschide la ora curentă (comportamentul tău actual)
    // SAU
    // initialDateForPicker = new Date();
    // initialDateForPicker.setHours(0, 0, 0, 0); // Deschide la miezul nopții (00:00) pentru a facilita selecția orelor vechi
  }
  ...
  setCurrentPickingInfo({
    ...
    slotIndex,
    fieldName,
    initialPickerDate: initialDateForPicker, // Transmitem ora inițială către picker
  });
  setPickerVisible(true);
};

  // Funcția care se apelează când utilizatorul selectează o oră
  const onTimeSelected = (event, selectedDate) => {
    setPickerVisible(Platform.OS === "ios"); // Pe iOS, picker-ul nu se închide automat după selecție
    // Pe Android, se închide, așa că putem seta direct false

    if (selectedDate) {
      // Formatăm ora în HH:MM
      ...
      // Actualizarea stării schedule în mod IMUTABIL
      setSchedule((prevSchedule) => {
        // 1. Copiază array-ul exterior al programului
        const newSchedule = [...prevSchedule];

        // 2. Copiază obiectul zilei specifice
        const newDay = { ...newSchedule[dayIndex] };

        // 3. Copiază array-ul timeSlots al zilei
        const newTimeSlots = [...newDay.timeSlots];

        // 4. Copiază obiectul slotului de timp specific și actualizează câmpul
        newTimeSlots[slotIndex] = {
          ...newTimeSlots[slotIndex],
          [fieldName]: formattedTime, // Aici actualizăm cu ora formatată
        };

        // 5. Asignează noul array timeSlots obiectului zilei copiate
        newDay.timeSlots = newTimeSlots;

        // 6. Asignează noul obiect al zilei array-ului programului copiat
        newSchedule[dayIndex] = newDay;

        return newSchedule; // Returnează noul array de program
      });
    }
  };
  ...
  //     const dayName = days[dayIndex]; // Presupun că ai array-ul `days` definit în componentă
  ...
  //        Alert.alert(
  //   "Interval Invalid",
  //   `În ziua ${dayName}, intervalul ${interval} este invalid. Ora de început trebuie să fie mai mică decât ora de sfârșit!`,
    ...
    for (let i = 0; i < schedule.length; i++) {
      ...
      for (let j = 0; j < timeSlots.length; j++) {
        ...
        if (startMinutes === endMinutes) {
          Alert.alert(
            `Interval invalid ${days[i]}`,
            `Intervalul ${interval} nu este valid.`
          );
          return;
        }
        ...
        if (endMinutes - startMinutes > 120) {
          Alert.alert(
            `Interval prea lung , ${days[i]}`,
            `Intervalul de irigare ${interval}  nu poate dura mai mult de 2 ore.`
          );
          return;
        }
      }
              ...
              <View style={styles.scheduleContainer}>
                ...
                {scheduledDays.map((dayIndex) => (
                  <View key={dayIndex} style={styles.dayScheduleContainer}>
                    ...
                    {pickerVisible && (
                      <DateTimePicker
                         value={currentPickingInfo.initialPickerDate || new Date()} 
                        mode="time"
                        is24Hour={true}
                        display="default"
