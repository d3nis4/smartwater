...
// Funcție pentru a gestiona schimbările în input-urile de timp (startTime/endTime)
// Asigură-te că TextInput-urile tale folosesc această funcție în onChangeText
const handleTimeChange = (dayIndex, slotIndex, fieldName, value) => {
...
const addTimeSlot = (dayIndex) => {
  setSchedule((prevSchedule) => {
    if (prevSchedule[dayIndex].timeSlots.length >= 3) {
      // Limită la 3 sloturi per zi
      Alert.alert(
        "Limită atinsă",
        "Nu poți adăuga mai mult de 3 intervale orare per zi."
      );
      return prevSchedule; // Nu modifica starea dacă limita e atinsă
    }
    ...
    // 3. Copiază array-ul timeSlots al zilei și adaugă noul slot
    newDay.timeSlots = [...newDay.timeSlots, { startTime: "", endTime: "" }];

    // 4. Asignează noul obiect al zilei array-ului programului copiat
    newSchedule[dayIndex] = newDay;
...
const removeTimeSlot = (dayIndex, slotIndex) => {
  setSchedule((prevSchedule) => {
    ...
    // 3. Creează un nou array timeSlots fără slotul de șters
    const newTimeSlots = newDay.timeSlots.filter(
      (_, idx) => idx !== slotIndex
    );

    // 4. Dacă ai șters ultimul slot, adaugă unul gol pentru a asigura că există întotdeauna un input
