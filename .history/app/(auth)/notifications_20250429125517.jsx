...
const Notifications = () => {
  ...
  useEffect(() => {
    ...
    const userDocRef = doc(db, "users", getSafeEmail(user.email));
    const unsubscribe = onSnapshot(userDocRef, (doc) => {
      if (doc.exists()) {
        const userData = doc.data();
        setSystemStatus({
          moisture: userData.soilHumidity || '--',
          pumpStatus: userData.pumpStatus || 'inactivă',
  ...
  useEffect(() => {
    ...
    const moisture = ref(db,`users/${safeEmail}` );
    const [moisture, setControls] = useState({});
    const unsubscribeControls = onValue(controlsRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        setMoisture({
          ragUmiditate: data.pragUmiditate || '--',
        })
        setControls({
    ...
    const unsubscribeLogs = onValue(dailyLogsRef, (snapshot) => {
      const data = snapshot.val();
      if (data) {
        // Găsim cea mai recentă înregistrare
