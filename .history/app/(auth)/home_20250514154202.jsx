...
const saveSettings = async () => {
  ...
  for (let i = 0; i < schedule.length; i++) {
    ...
    for (let j = 0; j < timeSlots.length; j++) {
      ...
      if (!startTime || !endTime) {
        Alert.alert("Interval incomplet", `Te rog să completezi toate orele pentru ziua ${[
      ...
      if (startTime >= endTime) {
        Alert.alert("Interval invalid", `În ziua ${[
          "Luni", "Marți", "Miercuri", "Joi", "Vineri", "Sâmbătă", "Duminică"
        ][i]}, ora de început trebuie să fie înaintea orei de sfârșit (ex: 08:00-10:00).`);
        return;
      }
    }
  }
