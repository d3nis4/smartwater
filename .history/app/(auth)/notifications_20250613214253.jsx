...
import { Colors } from "../../constants/Colors";
  // Pentru a primi date o singură dată, în loc de onValue folosește get:
...
/**
 * Componenta principală Notifications.
 * Afișează statistici, starea sistemului și jurnalele recente de irigare pentru utilizatorul curent.
 *
 * @component
 * @returns {JSX.Element} Interfața de notificări cu date de stare și irigare.
 */
export default function Notifications() {
  ...
  /**
   * Efectuează apeluri către Firebase pentru a obține și asculta modificările în:
   * - controale (pompa, program, etc.)
   * - jurnale de irigare zilnice
  ...
  useEffect(() => {
    ...
    const unsubscribeLogs = onValue(dailyLogsRef, (snapshot) => {
      ...
      if (data) {
        ...
        setLastWateringDate(filteredLogs.length > 0 ? filteredLogs[0].date : "N/A");
      }
    });

    const unsubscribeEmail = onValue(emailRef, (snapshot) => {
      const data = snapshot.val();
      if (data?.soilHumidity !== undefined && data.soilHumidity !== null) {
  ...
  /**
   * Calculează statistici precum: total irigări, durată medie, ziua cu cele mai multe irigări.
   */
  useEffect(() => {
    if (filteredData.length > 0) {
      ...
      filteredData.forEach((log) => {
        ...
        log.intervals.forEach((interval) => {
          ...
          if (durata > 0) totalDurata += durata;
        });
      });

      const durataMedie = totalIrigari > 0 ? totalDurata / totalIrigari : 0;
      const ziMaxIrigari = Object.entries(irigariPeZi).sort((a, b) => b[1] - a[1])[0]?.[0];
  ...
  /**
   * Filtrează datele de irigare în funcție de numărul de zile specificat.
   *
   * @param {Array} logs - Lista completă a jurnalelor.
   * @param {number} days - Numărul de zile pentru filtrare.
   */
  ...
  /**
   * Efectuează filtrarea la schimbarea intervalului de zile sau al jurnalelelor.
   */
  useEffect(() => {
  ...
  /**
   * Încarcă datele curente din Firebase o singură dată (fără onValue).
   */
  const loadData = () => {
    if (!user?.email) return;
    ...
    Promise.all([get(controlsRef), get(dailyLogsRef), get(emailRef)])
      .then(([controlsSnap, logsSnap, emailSnap]) => {
        const controlsData = controlsSnap.val();
        if (controlsData) {
          setControls({
            pumpMode: controlsData.pumpMode || "manual",
            pumpStatus: controlsData.pumpStatus || "off",
            program: controlsData.program || {},
          });
        }
        ...
        if (logsData) {
          ...
          const filteredLogs = Object.entries(logsData)
            .filter(([dateStr]) => new Date(dateStr) >= oneMonthAgo)
            .map(([dateStr, log]) => ({
              date: dateStr,
              intervals: log.intervals ? Object.values(log.intervals) : [],
        ...
        if (emailData?.soilHumidity !== undefined && emailData.soilHumidity !== null) {
          setMoisture(emailData.soilHumidity);
          setTemperature(emailData.temperature);
        }
      })
      .catch((error) => {
  ...
  /**
   * Reîncarcă datele la refresh manual.
   */
  const onRefresh = () => {
    loadData();
  };

  // returnul e partea de UI - nu necesită JSDoc, dar poate fi comentată în general dacă dorești.
  return (
    <ScrollView
    ...
    >
      ...
      {stats && (
        <View style={styles.statusCard}>
          ...
          <Text style={styles.bullet}>
            • Durată totală: {formatDuration(stats.durataTotala)}
          </Text>

          <Text style={styles.bullet}>
            • Durată medie per irigare: {formatDuration(stats.durataMedie)}
          </Text>
      ...
      <Text style={styles.sectionTitle}>{getTitleText(selectedRange)}</Text>
      <View
        style={{
          flexDirection: "row",
...
/**
 * Obiect StyleSheet pentru stilizarea componentei Notifications.
 */
