...
export default function Home() {
  ...
  const handlePumpModeChange = (newMode) => {
   ...
   // 1. Efectul principal pentru autentificare și citirea datelor Firebase
  useEffect(() => {
    const unsubscribeAuth = onAuthStateChanged(auth, (currentUser) => {
      ...
      const unsubscribeRealtime = onValue(userRef, (snapshot) => {
        ...
        // Actualizează setările de control
        if (data.controls) {
          ...
          setSavedPumpMode(data.controls.pumpMode || "Necunoscut"); // Poate fi redundant, dar e ok aici
        }
      ...
      return unsubscribeRealtime; // Curățare pentru ascultătorul `onValue`
    });

    return unsubscribeAuth; // Curățare pentru ascultătorul de autentificare
  }, []); // Rulează o singură dată la montarea componentei

  // 2. Efectul pentru modul inteligent (predicție)
  useEffect(() => {
    // Condiții pentru a activa logica smart mode
    if (!user?.email || pumpMode !== "smart" || overrideActive) {
      // Dacă condițiile nu sunt îndeplinite, nu facem nimic și curățăm eventualele intervale/ascultători active
      return; 
    }

    const safeEmail = getSafeEmail(user.email);

    // Rulează predicția imediat la activarea/modificarea condițiilor
    fetchSmartPrediction(safeEmail);

    // Setează interval pentru actualizare periodică (la fiecare 30 minute)
    const interval = setInterval(() => {
      fetchSmartPrediction(safeEmail);
    }, 30 * 60 * 1000); // 30 minute

    // Ascultă modificări în senzori pentru a declanșa noi predicții
    // Chiar dacă ascultătorul principal actualizează stările `moisture` și `temperature`,
    // păstrarea acestor ascultători direcți asigură că `fetchSmartPrediction` se declanșează
    // IMEDIAT ce datele senzorilor se schimbă în baza de date, fără a aștepta o actualizare
    // mai amplă a `userRef` sau o verificare a schimbării stării React.
    ...
    return () => {
      // Funcția de curățare: se rulează când efectul se demontează sau când dependențele se schimbă
      clearInterval(interval);
      moistureUnsub();
      tempUnsub();
    };
  }, [user, pumpMode, overrideActive]); // Dependențele care dictează când se activează/dezactivează logica smart mode
          ...
          <View style={styles.pumpContainer}>
            ...
            {pumpMode === "auto" && (
              <View style={styles.autoModeContainer}>
                <Text style={styles.autoModeText}>
                  Pompa va funcționa automat în funcție de pragul de umiditate setat.
                </Text>
