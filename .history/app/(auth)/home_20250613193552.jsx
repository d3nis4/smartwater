...
import { timeToMinutes, fetchSavedLocation } from "../../constants/functions";
...
export default function Home() {
  ...
  import { useRouter } from 'next/router'; // sau react-router-dom, în funcție de setup
...
useEffect(() => {
  const checkDeviceConfig = async () => {
    try {
      ...
      if (!configured) {
        router.replace('/deviceconfigure'); // redirect fără istoric în back stack
      }
    } catch (err) {
      console.error("Error checking device config:", err);
      setIsConfigured(false);
      router.replace('/deviceconfigure'); // sigur redirect dacă eroare
    } finally {
      setLoading(false);
    }
  };
